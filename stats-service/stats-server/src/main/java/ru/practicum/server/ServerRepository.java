...
public interface ServerRepository extends JpaRepository<EndpointHit, Long> {
    ...
    @Query("select new ru.practicum.server.dto.ViewStats(h.app, h.uri, count(h.ip) as hits) from EndpointHit as h " +
    ...
    @Query("select new ru.practicum.server.dto.ViewStats(h.app, h.uri, count(distinct h.ip) as hits) from EndpointHit as h " +
            "where h.timestamp between ?1 and ?2 group by h.uri, h.app order by hits desc ")
